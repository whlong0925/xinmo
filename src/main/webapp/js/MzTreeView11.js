...
function MzTreeView(Tname) {
	...
	this.toString = function() {
		...
		var str = "<A id='" + this.name
				+ "_RootLink' href='#' style='DISPLAY: none'></A>";
		...
		if (rootCN.length > 0) {
			...
			// this.name +".focusClientNode('"+ rootCN[0].id +"'); "+ this.name
			// +".atRootIsEmpty()",10);
			setTimeout(this.name + ".atRootIsEmpty();" + this.name
					+ ".focusClientNode('" + rootCN[0].id + "'); ", 10);
		}
		...
		return "<DIV class='MzTreeView' " + "onclick='" + this.name
				+ ".clickHandle(event)' " + "ondblclick='" + this.name
...
MzTreeView.prototype.setStyle = function() {
	/*
	 * width: 16px; \ height: 16px; \ width: 20px; \ height: 20px; \
	 */
	var style = "<style> DIV.MzTreeView DIV IMG{border: 0px solid #FFFFFF;} DIV.MzTreeView DIV SPAN IMG{border: 0px solid #FFFFFF;}";
	if (this.wordLine) {
		style += "DIV.MzTreeView DIV{ height: 20px;"
				+ (this.navigator == "firefox" ? "line-height: 20px;" : "")
				+ (this.navigator == "netscape" ? "" : "overflow: hidden;")
				+ "   }DIV.MzTreeView DIV SPAN { vertical-align: middle; font-size: 21px; height: 20px; color: #D4D0C8; cursor: default;}DIV.MzTreeView DIV SPAN.pm    {"
				+ "width: "
				+ (this.navigator == "msie" || this.navigator == "opera" ? "11"
						: "9")
				+ "px;"
				+ "height: "
				+ (this.navigator == "netscape" ? "9"
						: (this.navigator == "firefox" ? "10" : "11"))
				+ "px;"
				+ "font-size: 7pt; overflow: hidden; margin-left: -16px; margin-right: 5px;"
				+ "color: #000080; vertical-align: middle; border: 1px solid #D4D0C8;"
	 ...
	 * if(document.body) { var head = document.getElementsByTagName("HEAD")[0];
	 * head.innerHTML = head.innerHTML + style; } else
	 */
	...
	for ( var i = 0; i < RCN.length; i++) {
		...
		if (RCN[i].text == "") {
			...
			if (this.wordLine) {
				...
				node.iconExpand = RCN[i].childNodes.length > 1 ? HCN ? "PM0"
						: "L0" : HCN ? "PM3" : "L3";
...
MzTreeView.prototype.load = function(id) {
	...
	var reg = new RegExp("(^|" + _d + ")" + sid + d + "[^" + _d + d + "]+("
			+ _d + "|$)", "g");
...
MzTreeView.prototype.nodeInit = function(sourceIndex, parentId) {
	...
	var ctrlName = this.getAttribute(source, "ctrlName");// Rexliu modify on
	// 2007-1-23 13:50
	var ctrlChecked = this.getAttribute(source, "ctrlChecked"); // Rexliu
	// modifty on
	// 2007-1-23
	// 14:27
	var ctrlType = this.getAttribute(source, "ctrlType"); // Rexliu modify on
	// 2007-1-23 13:50
	if (ctrlType == "")
		ctrlType = "checkbox";
...
MzTreeView.prototype.nodeToHTML = function(node, AtEnd) {
	...
	var isMoudle = node.parentNode.parentId == "0";
	if (isRoot && node.icon == "")
		node.icon = "root";
	if (node.icon == "" || typeof (this.icons[node.icon]) == "undefined")		
	node.icon = isMoudle ? "folder" : "file";
	...
	if (!isRoot) {
		...
		if (this.wordLine) {
			HTML += "<SPAN>" + node.parentNode.childAppend
					+ (AtEnd ? "└" : "├") + "</SPAN>";
			if (HCN)
				HTML += "<SPAN class='pm' id='" + this.name + "_expand_" + id
						+ "'>+</SPAN>";
		} else {
			node.iconExpand = HCN ? AtEnd ? "PM2" : "PM1" : AtEnd ? "L2" : "L1";
			HTML += "<SPAN>"
					+ this.word2image(node.parentNode.childAppend)
					+ "<IMG "
					+ "align='absmiddle' id='"
					+ this.name
					+ "_expand_"
					...
					+ "' "
					+ "src='"
					+ this.icons[node.iconExpand].src
					+ "' style='cursor: "
					+ (!node.hasChild ? "" : (this.navigator == "msie"
							|| this.navigator == "opera" ? "hand" : "pointer"))
					+ "'></SPAN>";
		}
	}
	HTML += "<IMG " + "align='absMiddle' " + "id='" + this.name + "_icon_" + id
			+ "' " + "src='" + this.icons[node.icon].src + "'>";
	var sid = node.sourceIndex.substr(node.sourceIndex.indexOf(this.divider)
			+ this.divider.length);

	if (node.ctrl) {
		HTML += "<input type='" + node.ctrlType + "' name='" + node.ctrlName
				+ "' value=" + sid + " id='" + this.name + "_checbox_" + id
				+ "' style='height:15px;' onclick=\"" + this.name
				+ ".checkChild('" + id + "')\" "
				+ (node.ctrlChecked == 'true' ? 'checked' : '') + ">";
	}
	// Rexliu add on 2007-1-23 14:29
	HTML += "<A " + "class='MzTreeview' hideFocus " + "id='" + this.name
			+ "_link_" + id + "' " + "href='" + url + "' " + "target='"
			+ target + "' " + "title='" + node.hint + "' " + "onfocus=\""
			+ this.name + ".focusLink('" + id + "')\" " + "onclick=\"return "
			+ this.name + ".nodeClick('" + id + "')\">" + node.text + "</A>";
	HTML += "</NOBR></DIV>";
	if (isRoot && node.text == "")
		HTML = "";
...
MzTreeView.prototype.focusClientNode = function(id) {
	...
	if (a) {
		a.focus();
		var link = this.getElementById(this.name + "_link_"
				+ this.currentNode.id);
		if (link)
			with (link.style) {
				color = "";
...
MzTreeView.prototype.expand = function(id, sureExpand) {
	...
	if (area) {
		...
		if (img)
			img.src = !Bool ? icon.src
					: typeof (iconE) == "undefined" ? icon.src : iconE.src;
		...
		if (expand) {
			if (this.wordLine)
				...
				expand.src = !Bool ? exp.src
						: typeof (expE) == "undefined" ? exp.src : expE.src;
		}
		...
		area.style.display = !Bool ? "none" : "block";// (this.navigator=="netscape"
		// ? "block" : "");
		if (!node.isLoad) {
			this.load(id);
			if (node.id == "0")
			...
			if (node.hasChild && node.childNodes.length > 200) {
				...
				temp[temp.length - 1].innerHTML = "<DIV noWrap><NOBR><SPAN>"
						+ (this.wordLine ? node.childAppend + "└" : this
								.word2image(node.childAppend + "└"))
						+ "</SPAN>"
						+ "<IMG border='0' height='16' align='absmiddle' src='"
						+ this.icons["file"].src + "'>"
						+ "<A style='background-Color: "
						+ this.colors.highLight + "; color: "
						+ this.colors.highLightText
						+ "; font-size: 9pt'>请稍候...</A></NOBR></DIV>";
			} else
				this.buildNode(id);
		}
	}
...
MzTreeView.prototype.checkParent = function(id) {
	this.selParent(id);
	this.selChild(id);
};
...
MzTreeView.prototype.selChild = function(id) {
	...
	if (tcn.hasChild) {
		for ( var i = 0; i < tcn.childNodes.length; i++) {
			...
			var pckbox = this.getElementById(pckboxID);
			// 取子CHECKBOX
			var ckboxID = this.name + "_checbox_"
					+ this.node[id].childNodes[i].id;
			var ckbox = this.getElementById(ckboxID);
